...
import { Send, Video, VideoOff, Mic, MicOff, PhoneOff, User } from 'lucide-react'
...
export default function ConsultationRoom() {
  ...
  const createPC = () => {
    ...
    peerRef.current = pc
    pc.onicecandidate = (e) => { if (e.candidate) socket?.emit('webrtc_ice_candidate', { consultation_id: id, candidate: e.candidate }) }
  ...
  const handleAnswer = async ({ answer }) => { await peerRef.current?.setRemoteDescription(answer) }
  const handleICE    = async ({ candidate }) => { try { await peerRef.current?.addIceCandidate(candidate) } catch {} }
      ...
      <div className="mb-4">
        ...
        {consultation && (
          <p className="text-gray-500 text-sm">
            ...
            <span className={`font-semibold ml-1 ${consultation.status === 'active' ? 'text-brand-600' : 'text-amber-600'}`}>
              {consultation.status}
            </span>
          </p>
        )}
      ...
      <div className="grid lg:grid-cols-5 gap-4 h-[600px]">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 overflow-hidden flex flex-col">
          ...
          <div className="p-3 flex items-center justify-center gap-3 bg-gray-800">
            {!inCall ? (
              ...
              <>
                <button onClick={toggleAudio} className={`p-2.5 rounded-full ${audioEnabled ? 'bg-gray-600' : 'bg-red-500'} text-white`}>
                  {audioEnabled ? <Mic className="w-4 h-4" /> : <MicOff className="w-4 h-4" />}
                </button>
                <button onClick={toggleVideo} className={`p-2.5 rounded-full ${videoEnabled ? 'bg-gray-600' : 'bg-red-500'} text-white`}>
        ...
        <div className="lg:col-span-3 bg-white rounded-2xl border border-gray-100 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.map((msg, i) => {
              const isMe = msg.sender_id === JSON.parse(localStorage.getItem('oncosense-auth') || '{}')?.state?.user?.id
              return (
          ...
          <div className="p-3 border-t border-gray-100 flex gap-2">
            <input type="text" value={newMsg}
              ...
              onFocus={() => socket?.emit('typing', { consultation_id: id })}
